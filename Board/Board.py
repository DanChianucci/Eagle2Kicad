...
class Board(object):
    ...
    def __init__(self,node,converter=None):        
        ...
        libraries=node.find("drawing").find("board").find("libraries").findall("library")             
        elements=node.find("drawing").find("board").find("elements").findall("element")
        plain=node.find("drawing").find("board").find("plain")
    ...
    def getContacts(self,signals):
        ...
        for signal in signals:
            ...
            if self.signals.get(sigName)==None:                
            ...
            if not sigRefs==None:
                for sigRef in sigRefs:
                    ...
                    if self.contacts.get(element)==None:
                        self.contacts[element]={};
                    
                    if self.contacts[element].get(pad)==None:
    ...
    def writeEQUIPOT(self,outFile):
        ...
        for signal in self.signals:
            name=signal
            num=self.signals.get(name)
            outFile.write('$EQUIPOT\n')
            outFile.write('Na '+num+' '+name+'\n')
            outFile.write('St~\n')
